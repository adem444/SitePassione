...
{/* Header */}
<div className="bg-gradient-to-b from-[#2d4a1e] to-[#181818] rounded-t-2xl px-3 sm:px-6 md:px-8 lg:px-10 xl:px-12 pt-4 sm:pt-6 md:pt-8 pb-2 flex flex-wrap md:flex-nowrap items-start justify-between gap-4 md:gap-6 lg:gap-8">
  {/* Player Info */}
  <div className="flex flex-col gap-1 sm:gap-2 flex-1 min-w-[60%]">
    <h2
      className="text-white text-base sm:text-lg md:text-2xl lg:text-3xl xl:text-4xl font-extrabold uppercase tracking-tight truncate"
    ...
    <div className="flex items-center gap-1 sm:gap-2 mt-1">
      ...
      <span
        className="text-white text-xs sm:text-sm md:text-base lg:text-lg font-medium truncate"
    ...
    {player.stats?.atk && (
      <div className="mt-2">
        <span
          className="bg-[#181818] border border-[#629F3F] text-[#629F3F] font-bold text-[10px] sm:text-xs md:text-sm lg:text-base px-2 py-0.5 rounded uppercase"
          style={{ fontFamily: 'Gotham SSM, Bebas Neue, sans-serif' }}
        >
          ATK | {player.stats.value} VP
        </span>
      </div>
    )}
    ...
    {/* Status */}
    <div
      className="text-white text-[10px] sm:text-xs md:text-sm lg:text-base mt-2"
      style={{ fontFamily: 'Gotham SSM, Bebas Neue, sans-serif' }}
    >
      Statut : <span className="font-bold">{player.status}</span>
    </div>
  </div>

  {/* Jersey Image */}
  <div className="mt-3 md:mt-1 flex-shrink-0">
    <img
      src={player.jerseySrc}
      alt="Jersey"
      className="w-20 sm:w-24 md:w-28 lg:w-32 xl:w-36 object-contain drop-shadow-md"
    />
  </div>
</div>
          ...
          {player.name !== 'ADEM MHIRI' && (
            <div className="px-3 xs:px-6 md:px-8 pt-3 xs:pt-4 pb-5">
              ...
              <div className="flex flex-row gap-2 xs:gap-3">
                <button
                  className="flex-1 border border-[#629F3F] text-white font-bold py-2 xs:py-2.5 md:py-3 rounded-lg flex items-center justify-center gap-2 xs:gap-2.5 text-xs xs:text-lg md:text-xl uppercase cursor-pointer"
      ...
      {showReplacementModal && (
        <ReplacementModal
          ...
          onClose={() => { setShowReplacementModal(false); onClose && onClose(); }}
          player={player}
        />
      )}
    </>
