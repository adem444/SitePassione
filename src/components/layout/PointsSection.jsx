...
const getPointColor = (val) => {
  if (val.startsWith('+')) return 'text-[#629F3F]';
...
const SectionHeader = ({ children }) => (
  <tr>
    <td colSpan={2} className="py-2 pl-4 font-bold uppercase tracking-wide" style={{color:'#629F3F', fontFamily:'Bebas Neue, Gotham SSM, sans-serif', letterSpacing:'0.05em'}}>{children}</td>
  </tr>
);
        ...
        {tabList.map(tab => (
          <Tabs.Trigger
            ...
            className={`flex-1 px-4 py-3 text-xs sm:text-sm font-bold uppercase tracking-wide transition rounded-none focus:outline-none
              data-[state=active]:text-[#629F3F] data-[state=active]:border-b-2 data-[state=active]:border-[#629F3F] data-[state=inactive]:text-white`}
            style={{
              background: 'none',
